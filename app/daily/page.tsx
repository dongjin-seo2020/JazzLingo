...
function ProgressBar({ current, total }: { current: number; total: number }) {
  return (
    <div className="h-3 bg-[#E8D8C0] rounded-full overflow-hidden flex-1">
      <div
        className="h-full rounded-full transition-all duration-500 bg-gradient-to-r from-[#E8A020] to-[#F5C84A]"
  ...
  if (isDailyCompleted && !done) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#F5F0E8]">
        ...
        <h1 className="text-2xl font-black text-[#1A1224] mb-2">오늘은 완료했어요!</h1>
        <p className="text-[#6B5040] text-center mb-8">내일 다시 도전해요. 스트릭을 유지하세요! 🔥</p>
        <div className="bg-white rounded-2xl px-8 py-4 shadow-sm border border-[#E8D8C0] mb-8 text-center">
          <div className="text-3xl font-black text-[#FF9600]">{progress.streak}일</div>
          <div className="text-sm text-[#6B5040] font-bold">현재 스트릭 🔥</div>
        </div>
        <button
          onClick={() => router.push('/')}
  ...
  if (!started) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#F5F0E8]">
        ...
        <h1 className="text-3xl font-black text-[#1A1224] mb-2">오늘의 테스트</h1>
        <p className="text-[#6B5040] text-center mb-3">
          매일 바뀌는 10문제로 실력을 확인해요!
        </p>
        <div className="flex gap-3 mb-10">
          <div className="bg-white rounded-xl px-4 py-2 shadow-sm border border-[#E8D8C0] text-center">
            <div className="font-black text-[#E8A020]">10</div>
            <div className="text-xs text-[#6B5040]">문제</div>
          </div>
          <div className="bg-white rounded-xl px-4 py-2 shadow-sm border border-[#E8D8C0] text-center">
            <div className="font-black text-[#E8A020]">+100</div>
            <div className="text-xs text-[#6B5040]">XP</div>
          </div>
          <div className="bg-white rounded-xl px-4 py-2 shadow-sm border border-[#E8D8C0] text-center">
            <div className="font-black text-[#FF9600]">🔥</div>
            <div className="text-xs text-[#6B5040]">스트릭</div>
          </div>
        </div>
        <button
        ...
        <button onClick={() => router.push('/')} className="mt-4 text-[#6B5040] text-sm underline">
          나중에 할게요
        </button>
      </div>
    );
  ...
  if (done) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#F5F0E8]">
        ...
        <p className="text-[#6B5040] mb-6">10문제 중 {correctCount}문제 정답</p>
        <div className="flex gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl px-6 py-4 text-center shadow-sm border border-[#E8D8C0]">
            ...
            <div className="text-xs text-[#6B5040] font-bold">연속 🔥</div>
          </div>
        </div>
        {isPerfect && (
          <div className="bg-gradient-to-r from-[#E8A020] to-[#F5C84A] text-white font-black text-sm px-5 py-2.5 rounded-full mb-6">
            🏆 퍼펙트!
          </div>
        )}
        <button
    ...
    <div className={`min-h-screen flex flex-col bg-[#F5F0E8] ${isWrong ? 'animate-shake' : ''}`}>
      ...
      <div className="flex-1 flex flex-col px-4 py-4">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-bold text-[#E8A020] bg-[#FFF8E0] px-3 py-1 rounded-full">
            ⚡ 데일리 {index + 1}/{questions.length}
          </span>
          <span className="text-xs text-[#6B5040]">from: {q.lessonTitle}</span>
        </div>
