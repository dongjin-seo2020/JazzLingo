...
function XPBar({ xp }: { xp: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="text-sm font-bold text-[#E8A020] w-12">Lv.{level}</div>
      <div className="flex-1 h-3 bg-[#E8D8C0] rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-[#E8A020] to-[#F5C84A] rounded-full transition-all duration-700"
          style={{ width: `${progress}%` }}
        />
      </div>
      <div className="text-sm font-bold text-[#E8A020] w-16 text-right">{xp} XP</div>
    </div>
  );
}
  ...
  const nodeContent = (
    <div className={`flex items-center gap-4 ${isLeft ? 'flex-row' : 'flex-row-reverse'}`}>
      <div className="relative flex-shrink-0">
        ...
        {status === 'available' && (
          <div className="absolute -top-2 -right-2 bg-[#E8A020] text-white text-xs font-black px-2 py-0.5 rounded-full shadow animate-bounce">
            GO!
          </div>
        )}
      </div>
      ...
      <div
        className={`flex-1 bg-white rounded-2xl px-4 py-3 shadow-sm border border-[#E8D8C0] ${
          status === 'locked' ? 'opacity-40' : ''
        }`}
      >
        <div className="font-black text-base text-[#1A1224]">{lesson.title}</div>
        <div className="text-xs text-[#6B5040] mt-0.5">{lesson.description}</div>
        <div
          className="flex items-center gap-1 mt-1.5 text-xs font-bold text-[#E8A020]"
  ...
  return (
    <main className="pb-safe pt-4 px-4 bg-[#F5F0E8] min-h-screen">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-black text-[#1A1A2E]">JazzLingo 🎺</h1>
          <p className="text-xs text-[#6B5040]">Dongjin의 재즈 듀오링고</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-white rounded-full px-3 py-1.5 shadow-sm border border-[#E8D8C0]">
            <span>🔥</span>
      ...
      {/* XP Bar */}
      {ready && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-[#E8D8C0] mb-4 animate-pop-up">
          <XPBar xp={progress.xp} />
        </div>
      )}

      {/* Mascot + Greeting */}
      <div className="bg-white rounded-3xl p-4 shadow-sm border border-[#E8D8C0] mb-4 flex items-center gap-4">
        <WineMascot expression={completedCount > 0 ? 'happy' : 'excited'} size={72} className="flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="font-bold text-[#1A1224] text-sm leading-relaxed">{greeting}</p>
          <p className="text-xs text-[#6B5040] mt-1">
            {completedCount}/{lessons.length} 레슨 완료
          </p>
          <div className="h-2 bg-[#E8D8C0] rounded-full mt-2 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#E8A020] to-[#F5C84A] rounded-full transition-all duration-700"
      ...
      <Link href="/daily">
        ...
        >
          ...
          {!isDailyCompleted && (
            <span className="ml-auto text-[#E8A020] font-black text-sm">시작 →</span>
          )}
        </div>
      </Link>

      {/* Lesson Map */}
      <h2 className="font-black text-base text-[#1A1224] mb-4">🎵 레슨 맵</h2>
                ...
                <div className="flex justify-center py-1">
                  {[0, 1, 2].map((j) => (
                    <div
                      ...
                      style={{ background: prevDone ? '#E8A020' : '#D1D5DB' }}
                    />
                  ))}
                </div>
              )}
