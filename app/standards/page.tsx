...
const DIFF_COLORS: Record<number, { color: string; bg: string }> = {
  1: { color: '#58CC02', bg: '#E8F9DC' },
  2: { color: '#E8A020', bg: '#FFF8E0' },
  3: { color: '#C44B2A', bg: '#FDECEA' },
  4: { color: '#8B1A6B', bg: '#F5E8F5' },
};
  ...
  return (
    <main className="pb-safe pt-4 px-4 bg-[#F5F0E8] min-h-screen">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h1 className="text-2xl font-black text-[#1A1A2E]">📖 {t(lang, 'standards_title')}</h1>
          <p className="text-xs text-[#6B5040]">{t(lang, 'standards_subtitle')}</p>
        </div>
        <LangToggle />
      </div>
      ...
      <div className="relative mb-3">
        ...
        <input
          type="text"
          placeholder={t(lang, 'standards_search')}
          value={search}
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 mb-3">
        {([null, 1, 2, 3, 4] as (number | null)[]).map((value) => {
          const label = value === null ? t(lang, 'filter_all') : t(lang, `diff_${value}` as 'diff_1');
          return (
            <button
              key={String(value)}
      ...
      {/* Count */}
      <p className="text-xs text-[#9B8070] mb-3 font-bold">{t(lang, 'standards_count', filtered.length)}</p>

      {/* Standards list */}
      <div className="flex flex-col gap-3">
        {filtered.map((standard) => {
          const diff = DIFF_COLORS[standard.difficulty];
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="font-black text-[#1A1224]">{t(lang, 'no_results')}</p>
          <p className="text-sm text-[#6B5040] mt-1">{t(lang, 'no_results_sub')}</p>
        </div>
      )}
    </main>
  );
